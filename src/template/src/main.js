...
function init () {
  ...
  apiGroups.forEach(function (groupEntry) {
    ...
    api.forEach(function (entry) {
      if (groupEntry === entry.group) {
        ...
        if (apiProject.url) {
          if (fields.article.url.substring(0, 4).toLowerCase() !== 'http') {
  ...
  $('.sidenav').find('a').on('click', function (e) {
    ...
    const group = this.parentNode.getAttribute('data-group');
    $(".is-new").each(function() {
  ...
  $.urlParam = function (name) {
    const retValue = new RegExp('[\\?&amp;]' + name + '=([^&amp;#]*)').exec(window.location.href);
    return retValue && retValue[1] ? retValue[1] : null;
  };
    ...
    order.forEach(function (name) {
      if (splitBy) {
        elements.forEach(function (element) {
          ...
          if (key === name || parts[1] === name) { retValue.push(element); }
        });
      } else {
        elements.forEach(function (key) {
          if (key === name) { retValue.push(name); }
        });
      }
    });
    // Append all other entries that are not defined in order
    elements.forEach(function (element) {
      if (retValue.indexOf(element) === -1) { retValue.push(element); }
    });
    return retValue;
  }
  ...
  function sortGroupsByOrder (groups, order) {
    ...
    order.forEach(sortKey => {
      Object.keys(groups).forEach(name => {
        if (groups[name].replace(/_/g, ' ') === sortKey) { retValue.push(name); }
      });
    });
    // Append all other entries that are not defined in order
    Object.keys(groups).forEach(name => {
      if (retValue.indexOf(name) === -1) { retValue.push(name); }
    });
    return retValue;
  }
  initDynamic();
