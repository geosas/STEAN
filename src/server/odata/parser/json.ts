    ...
    export function charInJSON(value: Utils.SourceArray, index: number): number | undefined {
        ...
        if (escape && escape > index) {
            ...
            if (
                ...
                return escape + 1;
            if (Utils.equals(value, escape, "u") && Utils.required(value, escape + 1, Lexer.HEEDING, 4, 4)) return escape + 5;
            const escapeNext = Lexer.escape(value, escape);
            if (!escapeNext || escapeNext > escape) return escapeNext;
            const mark = Lexer.quotationMark(value, escape);
            if (!mark || mark > escape) return mark;
        } else {
