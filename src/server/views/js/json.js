JSONViewer = (function () {
    ...
    JSONViewer.prototype._createSimple = function (value) {
        ...
        var alink = document.createElement(type === "string" && value.startsWith("@http") ? "a": "label");
        var txt = value;

        if (type === "string") {
            if (value.startsWith("htt") && value.includes(params.version)) {
                type = "link";
                txt = value;
                // alink.href = this._createQueryLink(txt);
                alink.onclick = function () { clickLink(this); };
                alink.ondblclick  = function () { dblClickLink(this); };
                alink.className += "labelLink";
                spanEl.setAttribute("draggable", true);
                alink.innerHTML = txt;
            } else if (value.startsWith("@http") && value.includes(params.version)) {
                type = "link";
                alink.target = "_blank";
                txt = value.slice(1);
                alink.href = txt;
    ...
    JSONViewer.prototype._createLink = function (title, lvl) {
        var linkEl = document.createElement("label");
